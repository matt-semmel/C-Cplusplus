...
//		pthread_cond_wait(exchange->empty[message.destination], &mutex);
...
//		pthread_cond_wait(exchange->full[message.destination], &mutex);
		pthread_cond_wait(&exchange->full[message.destination], &mutex);
        }
//	commented out instead of deleted. when i signal/unlock here, ttest throws errors.
//	pthread_cond_signal(exchange->full);
//	pthread_mutex_unlock(&mutex);
...
//	interchanging full/empty in signal... no change...
	pthread_cond_signal(&exchange->empty[message.destination]);
...
// 2nd interchange... 1st one reverts us back to infLOOP
// as does 2nd interchange... THIS funct should WAIT ON FULL, SIGNAL ON EMPTY.
//	pthread_cond_wait(exchange->full[message.destination], &mutex);
...
//    pthread_cond_signal(exchange->full);
// interchanging empty with full to see if i'm being inconsistent, and in get_message.
    pthread_cond_signal(exchange->empty);
    pthread_mutex_unlock(&mutex);
...
// WAIT ON EMPTY, SIGNAL ON FULL
//	pthread_cond_wait(exchange->empty[
	pthread_cond_wait(&exchange->empty[id], &mutex);
    }
    // Hmmm... I wonder if another thread could have statched all messages before I got here!!!
    ret = mb_remove_item(exchange->buffer[id], message);
    pthread_cond_signal(&exchange->full[id]);
    pthread_mutex_unlock(&mutex);
