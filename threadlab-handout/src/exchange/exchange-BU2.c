...
//correct syntax below
	...
	//from lab instr	while (mb_is_empty(exchange->buffer[0])) {
	//	}
//	pthread_mutex_lock(&mutex);
        }
	pthread_cond_broadcast(exchange->empty, &mutex);
//	pthread_mutex_unlock(&mutex);
...
//	pthread_mutex_unlock(&mutex);
	...
	pthread_cond_signal(exchange->empty, &mutex);
	pthread_mutex_unlock(&mutex);
//	pthread_cond_signal(exchange->full, &mutex);
