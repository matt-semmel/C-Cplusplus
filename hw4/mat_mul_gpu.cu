...
__global__ void mm_gpu_shared(float* C, float* A, float* B, int n)
{
	...
	// LOL. I'm an idiot...
	//__shared__ float As[40][40];  // I couldn't get this to compile because it said it needed a constant value.
    //__shared__ float Bs[40][40];  // Correct results but slower until we hit 256 and 1024.
    ...
    for (int i = 0; i < n / block_size; i++) {
        ...
        As[threadIdx.y * block_size + threadIdx.x] = A[a_row * n + a_col];

        int b_row = i * block_size + threadIdx.y;
        int b_col = col;
        Bs[threadIdx.y * block_size + threadIdx.x] = B[b_row * n + b_col];

        __syncthreads();

        for (int j = 0; j < block_size; j++) {
            Cvalue += As[threadIdx.y * block_size + j] * Bs[j * block_size + threadIdx.x];
        }
...
int main(int argc, char** argv)
{
	...
	if(policy==1){ //CPU baseline
		...
		mm_gpu <<<dimGrid, dimBlock, sizeof(float) * block_size * block_size * 2>>> (d_C, d_A, d_B, N);
