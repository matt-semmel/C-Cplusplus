...
void copy_host_to_device(float* A, float* B, int n)
{
	...
	cudaMalloc(&d_A, (n * (n << 2)));
	cudaMalloc(&d_B, (n * (n << 2)));
	cudaMalloc(&d_C, (n * (n << 2)));
	...
	if (d_C != 0)
	{
		cudaMemcpy(d_A, A, ((n << 2) * n), 1);
		cudaMemcpy(d_B, B, ((n << 2) * n), 1);
		gettimeofday (&tv, &tz);
		time_memcpy = (((((double)tv.tv_usec / 1000000.0) + tv) - time_start) + time_memcpy);
		//return tv.tv_usec;
	}
