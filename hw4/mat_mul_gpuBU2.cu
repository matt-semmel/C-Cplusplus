...
void copy_host_to_device(float* A, float* B, int n)
{
	...
	/*cudaMalloc((void**) &d_A, (float)((n << 2) * n * n));
	cudaMalloc((void**) &d_B, (float)((n << 2) * n * n));
	cudaMalloc((void**) &d_C, (float)((n << 2) * n * n));
	...
	{
		assert(d_C);
		cudaMemcpy(d_A, A, (float)((n << 2) * n * n), cudaMemcpyHostToDevice);
		cudaMemcpy(d_B, B, (float)((n << 2) * n * n), cudaMemcpyHostToDevice);
	}*/
...
void copy_device_to_host(float* C, int n)
{
	...
	cudaMemcpy(d_C, C, (sizeof(float) * n * n), cudaMemcpyDeviceToHost);
	cudaFree(d_A);
	cudaFree(d_B);
	cudaFree(d_C);
...
int main(int argc, char** argv)
{
	...
	if(policy==1){ //CPU baseline
		...
		mm_gpu <<<dimGrid, dimBlock>>> (h_C, h_A, h_B, N);
